...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Protection settings round-trip, and enforcement covers story edits, package edits, and header, footer, and note lifecycle commands alike. Remote collaboration installs are authoritative and are not gated. Forms protection keeps form-field and addressed control content editable and the rest of the document read-only, and refuses suggesting mode, as Word disables Track Changes there. Read-only and comments-only protection open the document in viewing mode and refuse every other mode, so a refused edit is visible rather than silently dropped. Read-only matches Word, which refuses edits and comments alike. Comments-only is stricter than Word, which refuses edits but permits commenting; inserting a comment is not yet supported here. Under forms protection the fields stay editable, content inside an unlocked control stays editable, and Tab moves between fields. Word confines the caret to the fields, so an edit outside one cannot be attempted; this editor allows the caret outside and the store refuses the write there, though the command reports no change rather than a refusal and the controls stay enabled. Tracked-changes protection forces suggesting mode. An unenforced protection restricts nothing, matching Word after Stop Protection. Exception ranges (w:permStart) are not honored, so an edit inside one is refused rather than allowed. Review → Protect Document for Forms toggles filling-in-forms protection as one undoable edit. Lifting keeps the recorded restriction with enforcement off. The editor refuses to lift a password-protected document, and refuses to enforce over a document that declares a different restriction. Other protection modes cannot be set from the editor.',
  },
