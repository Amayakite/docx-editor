...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Register custom fonts with the fonts prop. Search standard, supplied, and provider-supported families in the font menu, including in blank documents. Selecting an unloaded family requests its bytes without resetting selection or undo. The editor fetches your URLs and checks each hash. Theme fonts come from the OOXML theme. Word-accurate wrap and pagination need real font bytes, so the optional @docx-editor.dev/fonts package supplies substitutes for common Word fonts. The five packaged document defaults target matching advance widths for covered glyphs; kerning can still change line breaks. defaultFonts() and packagedFonts() supply bytes for private editor font registration. customFonts() supplies configured app faces when the editor resolves fonts. The editor does not replace fonts in app headers or sidebars. Native fonts remain available for missing glyphs. The packagedFonts install option is deprecated and ignored, including true. installDefaultFontFaces is a deprecated no-op that resolves to 0 without fetching or registering fonts. Remove deprecated options and helper calls; pass font configuration through the editor fonts prop. Define app fonts separately. Century Gothic loads on demand and runs within 1%, so a wrap point can still move. A family with no metric-compatible substitute keeps your own measurement rather than being given an arbitrary face. packagedFonts() serves all six on demand and reaches no third party. It loads a family when a document names it, or when that family is the default face. googleFonts() adds a pinned open-licensed catalog once an app opts into the network. Both are resolvers with the same call shape, so useFonts and useDocxSource compose them as a list in precedence order. A later origin is told which faces an earlier one can already paint, so composing them never downloads the same face twice.',
  },
  {
